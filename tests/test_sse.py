...
_PATCH_DETECT = "incognito.api.events.detect"
_PATCH_VALIDATE = "incognito.api.events.validate"
...
async def test_events_happy_path_streams_all_stages_and_complete(
    ...
    with (
        patch(_PATCH_EXTRACT, return_value=[_SAMPLE_BLOCK]),
        patch(_PATCH_DETECT, return_value=[_SAMPLE_RAW_DETECTION]),
        patch(_PATCH_VALIDATE, return_value=[_SAMPLE_DETECTION]),
    ):
...
async def test_events_extraction_failure_emits_pipeline_error(
    ...
    with patch(_PATCH_EXTRACT, side_effect=PdfError("Failed to open PDF")):
        async with client.stream("GET", f"/api/events/{session_id}") as resp:
            assert resp.status_code == 200
            raw = await resp.aread()
...
async def test_events_detection_failure_emits_pipeline_error(
    ...
    with (
        patch(_PATCH_EXTRACT, return_value=[_SAMPLE_BLOCK]),
        patch(_PATCH_DETECT, side_effect=DetectionError("NER inference failed")),
    ):
        async with client.stream("GET", f"/api/events/{session_id}") as resp:
            assert resp.status_code == 200
            raw = await resp.aread()
